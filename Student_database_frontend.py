...
class Student:
    ...
    def __init__(self,root):
        ...
        def StudentRec(event):
            ...
            self.txtAdr.delete(0, END)
            self.txtAdr.insert(END,sd[4])
            self.txtAge.delete(0, END)
            self.txtAge.insert(END,sd[5])
            self.txtDoB.delete(0, END)
            self.txtDoB.insert(END,sd[6])
            self.txtGender.delete(0, END)
            self.txtGender.insert(END,sd[7])
            self.txtMobile.delete(0, END)
        ...
        DataFrameLEFT = LabelFrame(DataFrame, bd=1,width=1000,height=600, padx=20, bg="ghost white", relief=RIDGE, font=('arial',30,'bold'),text="Student Info\n")
        DataFrameLEFT.pack(side=LEFT)

        DataFrameRIGHT = LabelFrame(DataFrame, bd=1,width=450,height=300, padx=31, pady=3, bg="ghost white", relief=RIDGE, font=('arial',30,'bold'),text="Student Detail\n")
        DataFrameRIGHT.pack(side=RIGHT)

        #--------------------label with text

        self.lblStdID = Label(DataFrameLEFT, font=('arial', 20, 'bold'), text="Student ID:-",padx=3, pady=3,bg="Ghost White")
        self.lblStdID.grid(row=0, column=0,sticky=W)
        self.txtStdID = Entry(DataFrameLEFT, font=('arial', 20, 'bold'), textvariable=StdID, width=39)
        self.txtStdID.grid(row=0, column=1)

        self.lblfna = Label(DataFrameLEFT, font=('arial', 20, 'bold'), text="First Name:-", padx=3, pady=3,bg="Ghost White")
        self.lblfna.grid(row=1, column=0, sticky=W)
        self.txtfna = Entry(DataFrameLEFT, font=('arial', 20, 'bold'), textvariable=FirstName, width=39)
        self.txtfna.grid(row=1, column=1)

        self.lblSna = Label(DataFrameLEFT, font=('arial', 20, 'bold'), text="SurName:-", padx=3, pady=3,bg="Ghost White")
        self.lblSna.grid(row=2, column=0, sticky=W)
        self.txtSna = Entry(DataFrameLEFT, font=('arial', 20, 'bold'), textvariable=SurName, width=39)
        self.txtSna.grid(row=2, column=1)

        self.lblDoB = Label(DataFrameLEFT, font=('arial', 20, 'bold'), text="DoB:-", padx=3, pady=3,bg="Ghost White")
        self.lblDoB.grid(row=3, column=0, sticky=W)
        self.txtDoB = Entry(DataFrameLEFT, font=('arial', 20, 'bold'), textvariable=DoB, width=39)
        self.txtDoB.grid(row=3, column=1)

        self.lblAge = Label(DataFrameLEFT, font=('arial', 20, 'bold'), text="Age:-", padx=3, pady=3, bg="Ghost White")
        self.lblAge.grid(row=4, column=0, sticky=W)
        self.txtAge = Entry(DataFrameLEFT, font=('arial', 20, 'bold'), textvariable=Age, width=39)
        self.txtAge.grid(row=4, column=1)

        self.lblGender = Label(DataFrameLEFT, font=('arial', 20, 'bold'), text="Gender:-", padx=3, pady=3, bg="Ghost White")
        self.lblGender.grid(row=5, column=0, sticky=W)
        self.txtGender = Entry(DataFrameLEFT, font=('arial', 20, 'bold'), textvariable=Gender, width=39)
        self.txtGender.grid(row=5, column=1)

        self.lblAdr = Label(DataFrameLEFT, font=('arial', 20, 'bold'), text="Address:-", padx=3, pady=3,bg="Ghost White")
        ...
        self.lblMobile = Label(DataFrameLEFT, font=('arial', 20, 'bold'), text="Mobile:-", padx=3, pady=3,bg="Ghost White")
        ...
        studentList=Listbox(DataFrameRIGHT, width=41, height=13, font=('arial', 15, 'bold'), yscrollcommand=scrollbar.set)
        ...
        self.btnSearchData = Button(ButtonFrame, text="Search", font=('arial', 20, 'bold'), height=1, width=10, bd=4, command=searchDatas)
        self.btnSearchData.grid(row=0, column=4)
        self.btnUpdateData = Button(ButtonFrame, text="Update", font=('arial', 20, 'bold'), height=1, width=10, bd=4, command=updateDatas)
